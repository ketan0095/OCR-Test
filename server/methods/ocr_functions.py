...
def pytesseract_ocr(input, accuracy_flag=20):
    ...
    d = pytesseract.image_to_data(
        input, config="--psm 6", output_type=Output.DICT)
    n_boxes = len(d['level'])
...
def easy_ocr(img_path: str):
    ...
    for data in result:
        ...
        json_result["data"].append(data_to_append)
        cv2.rectangle(img_read, (int(data[0][0][0]), int(data[0][0][1])), (int(
            data[0][2][0]), int(data[0][2][1])), (0, 255, 0), 2)
...
if "__main__" == __name__:
    ...
    img = cv2.imread(
        r"C:\ketan personal\OCR Demo\server\temp\WhatsApp Image 2023-03-16 at 1.31.18 PM.jpeg")
